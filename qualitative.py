...
for file_name in demo_files:
    ...
    methods = {"Original Image": Path(r"tmp\original_image.jpg"), 
           "Manual Annotation": Path(r"data\crack_dataset_cleaned\混凝土桥梁裂缝optic_disc_seg\Annotations") / (file_name[:-3]+"png"), 
           "CircleGAN\n(2017)": Path(r"gan-result")/(file_name[:-4]+"_fake"+".png"), 
           "YOLOv8\n(2023)": Path(r"tmp\yolo_raw_result.jpg"), 
           "SAM\n(2023)": Path(r"tmp\SAM_prompting.jpg"), 
           "Proposed": Path(r"tmp\accepted_result.jpg")}
    j=0
    for method in methods.keys():
        file_dir = methods[method]
        # if method == "GAN":
